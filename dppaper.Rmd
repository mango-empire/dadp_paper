...
allowing existing Bayesian model to be extended to handle
noise infused data. The package is designed to integrate well with existing Bayesian workflows;
results can by analyzed using tools from the `rstan` ecosystem. Additionally, 
construction of the privacy-aware sampler is simplified 
through the specification of four independent modules.
The benefits are twofold: several privacy mechanisms --- these can even be from different privacy frameworks---
...
`dapper` may prove particularly useful to those engaged in studying the privacy utility trade-off of several
approaches or dealing with a privacy mechanism involves multiple transformations.
...
Its syntax should be `latent_f(theta)` where `theta` is a R vector
