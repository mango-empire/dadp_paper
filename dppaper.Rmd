...
confidential information [@Dwork2006]. In this framework, privacy is obtained
...
* `latent_f` is an R function that samples from the latent process. Its
...
* `st_f` is an R function which calculates a summary statistic. The optional
argument `add` is a flag which represents whether $st_f$ should be interpreted as 
...
```{r post-or-compare, fig.cap="comparison.",  echo = FALSE, fig.height=1.5, fig.width=5, fig.align='center'}
set.seed(1)
...
presented in [@Ju2022] where they apply a Laplace privacy 
...
will be provided later when constructing the `dapper` likelihood component.
...
matrix is a fixed, known constant. Hence to draw a sample from the data generating 
process we use the relation $f(x,y) = f(x)f(y \mid x)$.
    ```{r, echo = TRUE}
    latent_f <- function(theta) {
...
using a normal prior.
    ...
    ```
3. `st_f`: Although the summary statistic contains $x^Tx$, we do not
