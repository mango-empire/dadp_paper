...
`new_privacy()`. The function of `dapper_sample()` is to generate MCMC draws from
...
all information about the data generating process. And the role of `new_privacy()` is to
...
`dapper` implements probably mass and sampling functions
...
scale respectively. The `dapper` functions `ddnorm` and `rdnorm` provide the
...
```{r post-or-compare2, fig.cap= caption,  echo = FALSE, fig.height=3, fig.width=5, fig.align='center'}
caption <- "(Example 1) comparison between using dapper and a naive Bayesian anaylsis on the
noise infused data and the original confidential data." 
...
distribution to $6.25$ which will guarantee it is $2\log(3)$-differentially private^[$\dfrac{1}{2} \epsilon^2$-concentrated differential privacy implies $\left(\dfrac{1}{2}\epsilon^2 + \epsilon \sqrt{2\log(1/\delta)}, \delta\right)$-differential privacy [@Canonne2021]].  
...
SmartNoise by Microsoft [insert citation] and SafeTab [insert citation] by the US Census, for example,
...
established privacy mechanisms and those that have yet to be invented.
...
are notable weaknesses, which highlight important directions for future work.
Weaknesses include: 1) the requirement for the privacy mechanism to have a density 
2) the existence of a record additive statistic and 3) a relatively efficient sampler


The non-private posterior needs
