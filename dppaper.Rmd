...
interface for the data augmentation framework proposed by @Ju2022,
...
scientist are interested in learning about $\theta$ because it provides important information about the subject under investigation.
...
does not change much when one individual's data is changed. A common approach -- and the one dapper
...
for all neighboring databases where $d(x,x') = 1$, we have
\begin{align*}
...
# Methodology
Given privatized data, $s_{dp}$, the goal of Bayesian inference is to sample from the 
...
is just the usual posterior distribution given the confidential data $x$. The \CRANpkg{dapper}
...
sampler, the `dapper_sample()` function uses a `privacy` object to encapsulate
...
The `data_model` input is a privacy model object that is constructed 
...
a weaker criteria. The \CRANpkg{dapper} package implements probability mass and sampling functions
...
`ddlaplace` and `rdlaplace` provide similar features for the discrete Laplace distribution.
The implementations of these functions are not the safest and are meant to provide 
...
determined that the privacy budget is the issue, the pseudo-likelihood scheme proposed by @Andrieu2009
may offer significant speed ups. This scheme fits in the same data augmentation framework as `dapper` but is not implemented.
...
ensuing analysis or, if it does, addresses the analysis only for specific models.
...
established privacy mechanisms as well as those that have yet to be discovered.
...
3) the non-private posterior sampler needs to be efficient and 4) the privacy budget cannot be too small.
