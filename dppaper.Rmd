...
This function has the form `priv_f(sdp, sx)` where `sdp` is an R vector representing the
the value of $s_{dp}$ and `sx` is an R vector representing the value of the confidential summary statistic 
...
while `xi` and `sdp` are R vectors.
