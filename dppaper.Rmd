...
describing how to generate a new confidential record $x$, given $\theta$.
...
This function has the form `priv_f(sdp, sx)` where `sdp` is an R vector or matrix representing the
the value of $s_{dp}$ and `sx` is an R vector or matrix (whichever one matches `sdp`) representing the value of the confidential summary statistic 
