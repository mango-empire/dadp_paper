...
```{r, eval = FALSE}
tdf <- rstatix::counts_to_cases(adm_cnf)
tdf$Admit <- as.integer(tdf$Admit)
tdf$Gender <- as.integer(tdf$Gender)


pdf <- tdf
rflip <- as.logical(rbinom(nrow(tdf), 1, .5))
li <- sample(1:4, nrow(tdf), replace = TRUE)
lc <- list(c(1,1), c(2,2), c(1,2), c(2,1))
for(i in 1:nrow(tdf)) {
  if(rflip[i]) {
    pdf[i,] <- lc[[li[i]]]
  }
...
latent_f <- function(theta) {
  tmp  <- t(rmultinom(1, 4526, theta))
  tmp2 <- matrix(c(tmp[1], tmp[2], tmp[3], tmp[4]), nrow = 2, ncol= 2)
  tmp3 <- rstatix::counts_to_cases(tmp2)
  tmp4 <- cbind(as.integer(tmp3$Var1), as.integer(tmp3$Var2))
}
