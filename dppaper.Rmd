...
Typically
This function has the form `priv_f = f(arg1, arg2, ..)` where `arg1` is the
the value of $s_{dp}$ and `arg2` is the.
...
```{r post-or-compare, fig.cap="comparison.",  echo = FALSE, fig.height=3, fig.width=5, fig.align='center'}
set.seed(1)
confidential_data <- x
...
Ignoring duplicate entries, the global sensitivity is $\Delta = p^2 + 4p + 3$.
...
on $s_{dp}$. The density estimates show the naive method which ignores the privacy mechanism has bias and 
underestimates the variance.


```{r regression-compare, fig.cap = caption, echo = FALSE, fig.height=1.5, fig.width=5, fig.align='center'}
...
Compare dapper with confidential data.
```{r regression-data-compare, fig.cap="The pink density is the posterior fo", echo = FALSE, fig.height=1.5, fig.width=5, fig.align='center'}
coef_df <- dp_out$chain %>% 
  as_tibble() %>%
  pivot_longer(contains("beta"), names_to = "coefficient", values_to = "estimate") %>%
  mutate(data = "noisy")
