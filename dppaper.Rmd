...
date: "2023-11-01"
abstract: >
    This paper serves as a reference and introduction to using the R package \CRANpkg{dapper}. 
    \CRANpkg{dapper} encodes a sampling framework which allows exact Markov chain Monte Carlo 
    ...
    \CRANpkg{dapper} offers a significant step forward in providing 
...
adding independent, random error to each cell; the amount and type of which is determined by DP theory. When we fit a regression model to the noise infused data,
...
The difficulty lies in the marginal likelihood of the privatized data that results from correctly accounting for the injected 
noise. This function is often analytically intractable and as a result,
...
discrete Gaussian mechanisms, which also add noise scaled to the sensitivity;
however these mechanisms satisfy a different privacy criteria called *zero-concentrated differential privacy* (zCDP) [@Bun2016]. ^[Example 2 will consider another privacy framework called $(\epsilon, \delta)$-differential privacy which
is an extension of $\epsilon$-differential privacy to the case where the ratio bound
...
* `post_f()` is a function which makes a one-step draw from the private posterior. It has
...
for analyzing privatized count data. Currently, the U.S. Census Bureau
...
Pure $\epsilon$-differential privacy is a strong requirement
that can lead to poor data utility. For this reason the U.S. Census Bureau uses
zero concentrated differential privacy which is
a weaker criteria. The \CRANpkg{dapper} package includes probability mass and sampling functions
for the discrete Gaussian and discrete Laplace distributions [@Canonne2022]. These
are two potential mechanisms for deploying concentrated differential privacy,
and both are used in several current U.S. Census Bureau data products [@TumultLabs2022].
...
speed up repeated execution, an optional logical argument named `normalize` is provided. When set to `FALSE`, it forgoes calculating the normalizing constant, however the default value is
`TRUE`. Finally, the functions
...
to indicate there is bias favoring the males. The "paradox" arises
...
One of the privacy mechanisms used is the discrete Gaussian distribution. 
...
is the value the U.S. Census Bureau uses. 
...
```
For the public, the U.S. Census Bureau only releases aggregate cell counts along with
...
the discrete Gaussian induces considerably less bias when using the naive analysis.
...
$(x^Tx)^{-1}$ matrix is not positive definite. As a naive solution we use the algorithm proposed by @Higham1988 to find the closest positive semi-definite matrix
...
1) the privacy mechanism must have a closed-form density 2) a record additive 
statistic must be used to leverage \CRANpkg{dapper}'s full computational potential
3) the non-private posterior sampler needs to mix well and 4) the privacy budget cannot be too small.
To improve \CRANpkg{dapper}, future work could aim to relax some of these requirements.

# Disclosure Statement {-}

We are grateful for support from the Alfred P. Sloan Foundation. Jordan Awan's research was supported in part by NSF grant no. SES-2150615, awarded to Purdue University.
