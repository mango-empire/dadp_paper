...
theoretical guarantees despite appearing adequate. In one influential incident,
a privacy researcher was able use a publicly released medical data set
to deduce William Weld, then Governor of Massachusetts, medical diagnoses and prescriptions [@BarthJones2012].
...
unfeasible since the integral is usually high dimensional. 
...
the distribution of $x$. Differential privacy (DP) offers a more principled
...
in probability. To make our comparison we will set $\delta = 10^{-10}$, which
is the value the U.S. Census uses. To make a comparison,c we set the scale parameter in the discrete Guassian
distribution to $6.25$ which will guarantee $(2\log(3), 10^{-10})$-differential privacy.^[$\dfrac{1}{2} \epsilon^2$-concentrated differential privacy implies $\left(\dfrac{1}{2}\epsilon^2 + \epsilon \sqrt{2\log(1/\delta)}, \delta\right)$-differential privacy [@Bun2016]]  
...
Ignoring duplicate entries, the global sensitivity is $\Delta = p^2 + 4p + 3$.^[The original paper, @Ju2022, contains a computation error and mistakenly uses $\Delta = p^2 + 3p + 3$]
...
several stringent requirements for good performance that limit its potential:
