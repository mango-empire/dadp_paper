...
results can by analyzed using tools from the `rstan` ecosystem in a drop-in fashion. Additionally, 
...
privacy scheme, and the third example covers a linear regression model.
...
approach by introducing randomness in a way that the output distribution
...
been common practice in statistical disclosure control [@Dalenius1982], differential privacy
...
be made available without compromising privacy and incorporated into subsequent analyses.


The \CRANpkg{dapper} package provides a very flexible framework that can accommodate
the many different flavors a differential privacy; the main requirement being that 
the DP mechanism has a closed-form density.
The section on privacy mechanisms provides details about utility functions for working with dapper
...
running a Gibbs-like sampler. Like a Gibbs sampler, each of the $n$ components of $x$ is individually
...
The `dapper` package provides several utility functions 
for analyzing privatized count data. Currently, the U.S. Census 
...
that can lead to poor data utility. For this reason the U.S. Census uses
zero concentrated differential privacy which is
a weaker criteria. The `dapper` package implements probability mass and sampling functions
...
and both are, in fact, used in several current US Census data products [@TumultLabs2022].
...
uses with a way to quickly explore the behavior of DP mechanisms.
...
database, `tx`, is $\log(3/4)$ times the number of entries that match plus $\log(1/4)$ times
