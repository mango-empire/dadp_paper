...
The `dapper` package provides a set of tools for conducting
...
`dapper` may prove particularly useful to those engaged in studying the privacy utility trade-off or dealing with a privacy mechanism that involves multiple transformations.
...
The `dapper` package provides a very flexible framework that can accommodate
...
$\eta$ is said to be $(\epsilon, \delta)$-differentially private [@Dwork2006] if for all values of
...
for the the Laplace mechanism. Example 2, will cover an
...
however these mechanisms satisfy a different privacy criteria called *zero concentrated differential privacy* [@Bun2016].
...
without requiring a closed-form expression proportional to $p(s_{dp} \mid \theta)$.
...
`dapper` requires a valid draw and many sampler implementations violate 
this requirement. This is especially true for adaptive samplers like `rstan`'s HMC where
...
Pure $\epsilon$-differential privacy is a stringent requirement
...
scale respectively. The functions `ddnorm` and `rdnorm` provide the
...
speed up repeated execution, memoization is used via the \CRANpkg{memoise} package.^[Memoization is 
a programming technique used to cache repeated computations] Finally, the functions
...
corresponds to Dirichlet(1) distribution. A sample from the Dirichlet distribution
...
```{r trace-plot,  fig.cap="(Example 1) trace plots.", fig.height=3, fig.width=5, fig.align='center'}
plot(dp_out)
```
...
```{r post-or-density, fig.cap="(Example 1) posterior density estimate for the odds ratio using 16,000 MCMC draws.",  fig.height=3, fig.width=5, fig.align='center'}
tv <- dp_out$chain
...
To highlight the flexibility of `dapper` we reanalyze example 1
...
Because there are many users of Census data, a successful deployment requires 
a careful analysis of the privacy / utility trade-off and includes
studying how different mechanisms and privacy parameters affect
down stream analyses.

One of the mechanisms used is the discrete Gaussian distribution. 
...
The `dapper` package accommodates both the randomized response and
...
is the value the U.S. Census uses. Finally, setting the scale parameter in the discrete Guassian
...
data using `dapper`. Since the latent process
...
```{r post-or-density-dg, fig.cap="(Example 2) posterior density estimate for the odds ratio using 1,000 MCMC draws.",  fig.height=3, fig.width=5, fig.align='center'}
tv2 <- dp_out$chain
...
inference tools for privatized data. Despite the strengths of `dapper`, it has
...
statistic must be used to leverage `dapper`'s full computational potential
3) the non-private posterior sampler needs to be efficient and 4) the privacy budget cannot be too small.
To improve `dapper`, future work could aim to relax some of these requirements.
