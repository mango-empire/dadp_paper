...
To minimize the potential for bugs, there are a set of requirements which functions defining the four main components
must adhere to and are described below:


* `latent_f()` is an R function that samples from the parametric model 
describing how to generate a new confidential database $x$ given model parameters $\theta$.
Its syntax must be `latent_f(theta)` where `theta` is a R vector
representing the model parameters. This function
must work with the `init_par` argument of `dapper_sample()`. The output must be a $n \times p$ R matrix
where $n$ is the number of observations and $p$ is the dimension of a record $x$. 
The matrix requirement is strict, even if $p = 1$
`latent_f()` should return a $n \times 1$ matrix and not a R vector of length $n$.

* `post_f()` is a function which makes a one-step draw from the privacy-unaware posterior sampler. It has
the syntax `post_f(dmat, theta)`. Here `dmat` is a R matrix representing the confidential data
and `theta` is an R vector which serves as the initialization point for one-step draw. 
...
due to the large overhead associated with initializing the sampler. For these
...
* `priv_f()` is an R function that returns the log of the privacy mechanism density given
the noised infused summary statistics $s_{dp}$ and its potential true value $s(x, s_{dp}) := \sum_{i=1}^{n} t_i(x_i, s_{dp})$. 
This function has the syntax `priv_f(sdp, sx)` where `sdp` is an R vector or matrix representing the
the value of $s_{dp}$ and `sx` is an R vector or matrix representing $s(x, s_{dp})$.
...
terms in $t_i(x_i, s_{dp})$. Here the type class for `i` is an integer,
while `xi` is an R vector and `sdp` is an R vector or matrix. The return value
...
The three required inputs into `dapper_sample` function are the privacy model (`data_model`) 
whose construction is described in 4.1, and the value
of the observed privatized statistic (`sdp`) encoded as a R vector. The dapper
