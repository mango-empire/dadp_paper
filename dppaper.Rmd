...
`dapper` implements the probability mass and sampling functions for the 
discrete Gaussian and discrete Laplace distributions. The nomenclature and use
closely follow the base `stats` package. The probability mass functions are
named `ddnorm` and `ddlaplace` and the sampling functions are named
`rdnorm` and `rdlaplace`.

A random variable $X$ that follows a discrete Laplace distribution with
scale parameter $t$ has a probability mass function given by

\[
P[X = x] = \dfrac{e^{1/t} - 1}{e^{1/t} + 1} e^{|x|/t}.
\]

Random draws can be generated using an accept reject scheme and
`dapper` implements one such scheme outlined in []
...
show to how `dapper` is used operationally, we arbitrary set
...
```{r post-or-density-dg, fig.cap="(Example 2) posterior density estimate for the odds ratio using 16,000 MCMC draws.",  fig.height=3, fig.width=5, fig.align='center'}
tv <- dp_out$chain
or <- as.numeric((tv[,1] * tv[,4]) / (tv[,2] * tv[,3]))
