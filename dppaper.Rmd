...
concentrated differential privacy [@Canonne2021].
The strength of `dapper` comes from the fact we can analyze both scenarios under the
same framework.
...
current privacy 
parameters^[There is only a direct relationship between zCDP and $(\epsilon, \delta)$-differential privacy. To make comparisons between pure $\epsilon$-differential privacy and zCDP, the Census uses the value $\delta = 10^{-10}$.], setting
the scale parameter $\sigma = 6.25$ implies a pure $\epsilon$-differential
privacy budget of $2\log(3)$^[@Canonne2021 showed $\dfrac{1}{2} \epsilon^2$-concentrated differential privacy implies $\left(\dfrac{1}{2}\epsilon^2 + \epsilon \sqrt{2\log(1/\delta)}, \delta\right)$-differential privacy.]. Using the `rdnorm` function allows
...
how to construct `st_f` and `priv_f`. [MENTION TOTAL COUNTS ARE ALSO GIVEN]
