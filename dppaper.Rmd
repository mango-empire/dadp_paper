...
however these mechanisms satisfy a different flavor of DP called concentrated DP.
...
that can lead to poor data utility. For this reason the census data is privatized 
under a different framework called concentrated differential privacy [@Bun2016] which is
a weaker definition. The `dapper` package implements probability mass and sampling functions
...
speed up repeated execution, memoization is used via the \CRANpkg{memoise} package. Finally, the functions
...
zero concentrated differential privacy (zCDP) [@Canonne2021].
`dapper` accommodates both the randomized response and
the discrete Gaussian mechanisms, allowing us to compare the impact 
of the two approaches.

To begin comparing the two approaches, we need to set comparable privacy parameters.
Since the two frameworks use different metrics, there does not exist
a direct comparison^[There is only a direct relationship between zCDP and $(\epsilon, \delta)$-differential privacy. To make comparisons between pure $\epsilon$-differential privacy and zCDP, the Census uses the value $\delta = 10^{-10}$.]. However we can, in a sense, choose zCDP parameters that ensure
...
distribution to $6.25$ which will guarantee it is $2\log(3)$-differentially private^[$\dfrac{1}{2} \epsilon^2$-concentrated differential privacy implies $\left(\dfrac{1}{2}\epsilon^2 + \epsilon \sqrt{2\log(1/\delta)}, \delta\right)$-differential privacy [@Bun2016]].  
