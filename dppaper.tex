...
to deduce William Weld, then Governor of Massachusetts, medical diagnoses and prescriptions (Barth-Jones 2012).
...
The \texttt{dapper} package provides a very flexible framework that can accommodate
...
however these mechanisms satisfy a different privacy criteria called \emph{zero concentrated differential privacy} (Bun and Steinke 2016).
...
is just the usual posterior distribution given the confidential data \(x\). The \texttt{dapper}
...
scale respectively. The functions \texttt{ddnorm} and \texttt{rdnorm} provide the
...
speed up repeated execution, memoization is used via the \CRANpkg{memoise} package.\footnote{Memoization is
...
The \texttt{dapper} package accommodates both the randomized response and
...
distribution to \(6.25\) which will guarantee \((2\log(3), 10^{-10})\)-differential privacy.\footnote{\(\dfrac{1}{2} \epsilon^2\)-concentrated differential privacy implies \(\left(\dfrac{1}{2}\epsilon^2 + \epsilon \sqrt{2\log(1/\delta)}, \delta\right)\)-differential privacy (Bun and Steinke 2016)}
...
#>   variable  mean median     sd    mad     q5   q95  rhat ess_bulk ess_tail
#>   <chr>    <dbl>  <dbl>  <dbl>  <dbl>  <dbl> <dbl> <dbl>    <dbl>    <dbl>
#> 1 pi_11    0.315  0.313 0.0266 0.0269 0.273  0.360  1.00     660.     684.
#> 2 pi_01    0.286  0.285 0.0267 0.0269 0.243  0.329  1.00     611.     622.
#> 3 pi_10    0.107  0.106 0.0197 0.0200 0.0767 0.141  1.00     428.     737.
#> 4 pi_00    0.292  0.292 0.0262 0.0261 0.251  0.339  1.00     666.     867.
\end{verbatim}
...
Ignoring duplicate entries, the global sensitivity is \(\Delta = p^2 + 4p + 3\).\footnote{The original paper, Ju et al. (2022), contains a computation error and mistakenly uses \(\Delta = p^2 + 3p + 3\)}
...
inference tools for privatized data. Despite the strengths of \texttt{dapper}, it has
...
statistic must be used to leverage \texttt{dapper}'s full computational potential
3) the non-private posterior sampler needs to be efficient and 4) the privacy budget cannot be too small.
To improve \texttt{dapper}, future work could aim to relax some of these requirements.
...
\leavevmode\vadjust pre{\hypertarget{ref-BarthJones2012}{}}%
Barth-Jones, Daniel C. 2012. {``The {`Re-Identification'} of Governor William Weld's Medical Information: A Critical Re-Examination of Health Data Identification Risks and Privacy Protections, Then and Now.''} \emph{SSRN Electronic Journal}. \url{https://doi.org/10.2139/ssrn.2076397}.
