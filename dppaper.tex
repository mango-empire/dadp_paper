...
This paper will go over the dapper package. It implements
the Bayesian framework described in {[}-{]}. The intent of the dapper package is to help privacy researchers
evaluate how their analysis might be affected by directly infusing data with noise.
...
is to learn some characteristic about the population it was collected
from. To accomplish this task, a common approach is to assume the population
is represented by some statistical model \(f( \cdot \mid \theta)\) with \(\theta\)
having some relevant meaning to the scientific question at hand. In this case,
