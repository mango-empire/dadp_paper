...
of summary statistics while hiding sensitive characteristics of the data upon which these statistics are based off of. For example, a data aggregation service might
collect salary information for the purpose of helping its users negotiate salaries.
Such information is typically considered sensitive, so there is a strong desire
to keep the salary information of individuals anonymous. Differential privacy
offers a principle way of evaluating how random noise can be injected to product
individual salary information.

Compared to older statistical disclosure methods, such as random swapping. The
hallmark of DP based methods is the design of the privacy mechanism is
known by both the data curator and the data user. In the salary aggregation example,
the data aggregation service would make public, the process by which they
added noise.
...
studied approach and is the case which this paper address.
In both cases, the added noise induces biases which
must be accounted for in order to perform sound inference.

The goal of dapper, is to provide a tool to help privacy researchers
evaluate how their analysis might be affected by directly infusing
the confidential data with noise.
